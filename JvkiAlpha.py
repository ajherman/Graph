...
max_v = 23
#idx_set = [(v,k,i) for v in range(1,max_v) for k in range(1,v//2+1) for i in range(k)]
idx_set = [(v,k,k-1) for v in range(13,23) for k in range(1,v//2+1)]
#idx_set = [(max_v,k,i) for k in range(1,max_v) for i in range(k)]
...
for v,k,i in idx_set:
        ...
        maxIndSet = fastFindIndSet(B,niters,ntrials,start=-2,stop=2.5)
