...
## Load arrays
#alphas = np.loadtxt("IndependentSets/KneserAlphasTentative.txt",dtype=dtype)
#max_v = np.shape(alphas)[0]
#maxIndSets = np.load("IndependentSets/maxKneserIndSetsTentative.npy")
...
for v in range(max_v):
    for k in range(1,v//2+1):
        ...
        if alpha>alphas[v,k]:
            ...
            alphas[v,k] = alpha
            alphas[v,v-k] = alpha
            np.savetxt("IndependentSets/KneserAlphasTentative.txt",alphas,fmt='%5d')
            np.save("IndependentSets/maxKneserIndSetsTentative.npy",maxIndSets)
