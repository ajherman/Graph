...
def main(args):
    ...
    A_comp = 1-A-torch.eye(A.shape[0],device=device)
    A = A.to(device) # Necessar?
    A_comp = A_comp.to(device)
    N = A.shape[0] # Number of vertices
    w1,w2,w3=1,1,1
    M = w1*A + w2*torch.eye(N,device=device) + w3*A_comp
    q=1.0
    ...
    for step in range(n_steps+1):
        ...
        if representation == "softmax":
            ...
            p = torch.pow(torch.abs(x),q)
        
        if make_movie:
            points.append(p.detach()) # Collect points

        multi_loss = 0.5*torch.sum((mask*M)*(p@p.transpose(-1,-2)),dim=(-1,-2)) # Covariance matrix
        multi_loss -= torch.sum(p@p.transpose(-1,-2),dim=(-1,-2)) # Covariance matrix
        ...
        if step%500==0:
            ...
            edge_violations = 0.5*torch.sum((mask*A_comp)*(colorings@colorings.transpose(-1,-2)),dim=(-1,-2)) # Covariance matrix
            edge_violations -= torch.sum(colorings@colorings.transpose(-1,-2),dim=(-1,-2)) # Covariance matrix
        ...
        if representation == "quantum":
            x.data = torch.nn.functional.normalize(x.data,dim=-2,p=2)
        elif representation == "literal":
            x.data = torch.nn.functional.normalize(x.data,dim=-2,p=1)
        elif representation == "experimental":
            x.data = torch.nn.functional.normalize(x.data,dim=-2,p=q)
